...
function initializeClock(clockElement, endtime) {
  ...
  const millisecondsSpan = clockElement.querySelector('.milliseconds');

  const minutesLabel = clockElement.querySelector('.minutes-label');
  const secondsLabel = clockElement.querySelector('.seconds-label');
  const millisecondsLabel = clockElement.querySelector('.milliseconds-label');
  ...
  function updateClock() {
    ...
    //   'секунд',
    // ]);
    // millisecondsLabel.innerHTML = 'милли';
...
// Пример: 3 минуты от текущего времени
const deadline = new Date(Date.parse(new Date()) + 3 * 60 * 1000);

// Запуск таймеров на всех элементах с классом "countdown"
document.querySelectorAll('.countdown').forEach(clock => {
